...
> **⚠️ 上游项目的许可证描述不清。在部署、再分发，或依赖本仓库的任何内容之前，请先读这一节。**

**原项目并未对本代码授予任何许可。** 就两个仓库目前的状态而言：

- **没有 `LICENSE` 文件** —— 上游没有，本分支也没有
- `agent/src-tauri/Cargo.toml` 以及所有 `package.json` 中**均未声明 `license` 字段**
- 上游自己的 README 写着 *"Apache-2.0 (pending — will be locked in before v0.2.0)"*
  —— 这个目标并未兑现
- `docs/SECURITY.md` 只把 Apache-2.0 列为**推荐**，说明「公开正式发布前须决定」，
  并提到 MIT 作为备选

没有许可证，默认即**保留所有权利**：实际上并未向任何人授予使用、修改或再分发的权限
—— 没有授予本分支，也没有授予其他人。

本分支既没有解决、也不声称能解决这一点；它继承了这个不确定性，而本分支所有者无法授予
自己并不持有的权利。以上是对两个仓库中文件现状的陈述，不构成法律意见 —— 如果这件事对你
重要，请在依赖任何许可证之前，去问原作者，或者问律师。
